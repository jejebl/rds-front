...
const SubmitedTransactionsPool1 = (data) => {
  ...
  return (
          <>
            ...
            <button className="deleted_button" onClick={() => data.deleteTransactionPool1(data.data.id)}>
              Delete
            </button>
            </>
            :''}
            ...
            <>
            <div className='submited_info'>
              <p>Transaction submited Pool 1: {data.data.name + ' ' + ethers.utils.formatEther(data.data.value, 18) + ' (' + data.data.addr + ')' + ' to: ' + data.data.destination}</p>
            </div>
            <button className="deleted_button" onClick={() => data.deleteTransactionPool1(data.data.id)}>
              Delete
            </button>
...
export default SubmitedTransactionsPool1
